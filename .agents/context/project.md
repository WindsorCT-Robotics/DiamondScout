...
API, which will use Fable.Remoting to communicate with the backend. The
RCL will use this shim library's functions to implement component behaviors.
...
using an API key. The key is formatted as such: UserName:APIKey where UserName is the FRC username and APIKey is the
...
Aggregates shall be defined using the `Aggregate` type.
...
The RCL will use the shim library to implement component behaviors. It will be an F# library that will be used to
...
Type Extensions used outside the file the original type is defined in do not appear to C# consumers. It is imperative
that the type, its `Functional` module, and the Type Extension are all defined in the same file in that order.

F# discriminated unions should include helper functions for matching on the union to be used by C# consumers. While
they don't need to be named `Match`, they should expose the functionality of the `match` keyword as a member of the
type. The function name should use ubiquitous language and be descriptive of the functionality. (e.g., `Handle` for
matching on a discriminated union defining errors.)
...
new instances instead of modifying the original. `Event` types should always have private constructors, as `Event`s
should only be created by deciders.
