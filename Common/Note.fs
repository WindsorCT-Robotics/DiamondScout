...
module Functional =
    ...
    module Note =
        ...
        let private evolve note event =
            ...
            | NoteState.Created note, Edited newContent -> { note with Content = newContent } |> NoteState.Created
...
type Note with
    static member Create userId dateAdded content =
        (Note.Create(userId, dateAdded, content), NoteState.NotCreated)
        ||> Note.definition.Decide
        |> Validation.map _.ToReadOnlyList()

    static member Edit content note =
        (Note.Edit content, note)
        ||> Note.definition.Decide
        |> Validation.map _.ToReadOnlyList()

    static member Evolve note (events: IReadOnlyList<Note.Event>) =
        events |> _.FromReadOnlyList() |> foldEvents Note.definition note
